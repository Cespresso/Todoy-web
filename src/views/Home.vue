<template>
  <v-layout row>
    <v-flex xs12 sm10 offset-sm1>
      ...
      <v-btn color="pink" dark fab fixed bottom right @click="handleAddTodoBtn()">
        <v-icon>add</v-icon>
      </v-btn>
    </v-flex>
  </v-layout>
...
export default class Home extends Vue {
  ...
  private get todo() {
    let todos = this.$store.state.todo;
    todos.items = _.orderBy(
      // sliceで新しいオブジェクトを作成
      this.$store.state.todo.items.slice(),
      ["id"],
      ["asc"]
    );
    return todos;
